(function(window, document, Laya) {
	...
	var RunDriver = laya.utils.RunDriver,
	...
	var BMiniAdapter = (function() {
		...
		BMiniAdapter.init = function(isPosMsg, isSon) {
			...
			BMiniAdapter.systemInfo = swan.getSystemInfoSync();
		...
		BMiniAdapter.idx = 1;
		__static(BMiniAdapter, ['nativefiles', function() {
	...
	var MiniFileMgr$1 = (function() {
		...
		MiniFileMgr.readFile = function(filePath, encoding, callBack, readyUrl, isSaveFile, fileType) {
			...
			(isSaveFile === void 0) && (isSaveFile = false);
			(fileType === void 0) && (fileType = "");
			if (filePath && filePath.indexOf("data:image/png;base64") == 0) {
				callBack != null && callBack.runWith([1, fileType]);
				return;
			}
			MiniFileMgr.fs.readFile({
				filePath: filePath,
		...
		MiniFileMgr.setNativeFileDir = function(value) {
			MiniFileMgr.fileNativeDir = swan.env.USER_DATA_PATH + value;
		}
		...
		__static(MiniFileMgr, ['fs', function() {
			return this.fs = swan.getFileSystemManager();
		}, 'wxdown', function() {
			return this.wxdown = swan.downloadFile;
		}]);
		return MiniFileMgr;
	})()
	...
	var MiniInput$1 = (function() {
		...
		MiniInput._createInputElement = function() {
			...
			swan.onWindowResize && swan.onWindowResize(function(res) {
		...
		MiniInput.wxinputFocus = function(e) {
			...
			BMiniAdapter.window.swan.showKeyboard({
		...
		MiniInput.hideKeyboard = function() {
			BMiniAdapter.window.swan.offKeyboardConfirm();
			BMiniAdapter.window.swan.offKeyboardInput();
			BMiniAdapter.window.swan.hideKeyboard({
				success: function(res) {
					console.log('隐藏键盘')
				},
				fail: function(res) {
	...
	var MiniLoader$1 = (function(_super) {
		...
		MiniLoader.onReadNativeCallBack = function(encoding, url, type, cache, group, ignoreCache, thisLoader, errorCode, data) {
			...
			if (!errorCode) {
				...
				if (!BMiniAdapter.isZiYu && BMiniAdapter.isPosMsgYu && type != "arraybuffer") {}
				thisLoader.onLoaded(tempData);
			} else if (errorCode == 1) {
